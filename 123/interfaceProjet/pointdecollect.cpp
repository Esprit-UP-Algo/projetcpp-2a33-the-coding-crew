...
QMap<QString, int> PointDeCollect::getAgeStatistics() {
    QMap<QString, int> statistics;

        QSqlQuery query;
        query.prepare("SELECT COUNT(*) AS count FROM EMPLOYES WHERE quntiter < 30");
        if (query.exec() && query.next()) {
            statistics["Moins de 30 quntiter"] = query.value("count").toInt();
        }

        query.prepare("SELECT COUNT(*) AS count FROM EMPLOYES WHERE quntiter >= 30 AND quntiter <= 40");
        if (query.exec() && query.next()) {
            statistics["30-40 quntiter"] = query.value("count").toInt();
        }

        query.prepare("SELECT COUNT(*) AS count FROM EMPLOYES WHERE quntiter > 40");
        if (query.exec() && query.next()) {
            statistics["Plus de 40 quntiter"] = query.value("count").toInt();
        }
