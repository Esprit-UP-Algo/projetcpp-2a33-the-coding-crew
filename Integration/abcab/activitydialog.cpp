...
void activitydialog::on_inizio_userTimeChanged(const QTime &time)
{
    if(time >= ui->fine->time()){
        ui->Error->setText("<font color='red'> Mauvais moment <font>");
        ui->buttonBox->buttons().at(0)->setEnabled(false);
    }else{
                ...
                case 0:
                    if(punt->at(i).data==data || ui->fine->date() > data){
                        if((inizio >= pIn && inizio<pF) || (fine > pIn && fine < pF)|| (inizio <pIn && fine >= pF)){
                            ui->Error->setText("<font color='red'>Il chevauche " + punt->at(i).titolo + " da " + punt->at(i).data.toString() +"<font>");
                            return false;
                        }
                    }
                    break;
...
void activitydialog::finishCheck(){
    ...
    if(ui->ripeti->currentIndex() > 0 && date == data){
        ui->Error->setText("<font color='red'>Date de fin = Date de début, pas de répétition <font>");
        err = false;
    }
...
void activitydialog::on_tipo_activated()
{
    this->setAttivita();
}
